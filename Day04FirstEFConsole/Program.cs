    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                //updating equivalent of update =fetch then modify,save changes

                Person p2 = (from p in db.People where p.Id == 3 select p).FirstOrDefault<Person>();
                if(p2 !=null)
                {

                    p2.Name = "Alabama" + (random.Next(1000) + 1000);
                    db.SaveChanges();
